 ...
 */ function main() {
    (0, _slog.slog).info("Using database", [
        "database",
        `${undefined}${undefined}`
    ]);
}
/* Register event handler to run after the page is fully loaded. */ document.addEventListener("DOMContentLoaded", ()=>{
    main();
});

},{"./slog":"jExiC"}],"jExiC":[function(require,module,exports) {
/**
...
class SLog {
   ...
   */ debug(msg, ...pairs) {
        if (this.level <= this.LevelDebug) this.log("DEBUG", msg, pairs);
    }
    /**
   ...
   * @param pairs key/value pairs to be logged
   */ info(msg, ...pairs) {
        if (this.level <= this.LevelInfo) this.log("INFO", msg, pairs);
    }
    /**
   * Log warning message.
   *
   ...
   * @param pairs key/value pairs to be logged
   */ warn(msg, ...pairs) {
        if (this.level <= this.LevelWarn) this.log("WARNING", msg, pairs);
    }
    /**
   * Log error message.
   *
   ...
   * @param pairs key/value pairs to be logged
   */ error(msg, ...pairs) {
        if (this.level <= this.LevelError) this.log("ERROR", msg, pairs);
    }
    constructor(){
        /** Debug log level. */ this.LevelDebug = -4;
        /** Info log level. */ this.LevelInfo = 0;
        /** Warning log level. */ this.LevelWarn = 4;
        /** Error log level. */ this.LevelError = 8;
        /** Current log severity level. Should only be changed by setLevel. */ this.level = this.LevelInfo;
    }
}
const slog = new SLog();
...
},{}]},["jljck","jeorp"], "jeorp", "parcelRequire562a")
